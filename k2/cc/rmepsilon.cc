 ...
 */
static int32_t MapStates(const kk::Fsa &fsa_in, std::vector<char> *non_eps_in,
  ...
  for (const auto &arc : fsa_in) {
    ...
    if (arc.label != kk::kEpsilon) non_eps_in_values[arc.dest_state] = 1;
  }
...
static void TraceBackRmEpsilons(
    std::map<int32_t, kk::LogSumTracebackState *> *curr_states,
  ...
  kk::LogSumTracebackState *state_ptr = curr_states->rbegin()->second;
  ...
  while (!state_ptr->prev_elements.empty()) {
    ...
    for (const auto &link : state_ptr->prev_elements) {
      ...
      kk::LogSumTracebackState *prev_state = link.prev_state.get();
      ...
      if (result.second) {
        ...
        prev_state->backward_prob =
            kk::LogAdd(new_backward_prob, prev_state->backward_prob);
      }
    }
 ...
 */
static void TraceBackRmEpsilons(
    std::map<int32_t, kk::MaxTracebackState *> *curr_states,
  ...
  kk::MaxTracebackState *state_ptr = curr_states->begin()->second;
