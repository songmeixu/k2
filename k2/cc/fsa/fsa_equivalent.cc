 ...
 */
static bool Connect(const kk::Fsa &fsa_in, kk::FsaCreator *fsa_out,
                    std::vector<int32_t> *arc_map = nullptr) {
  CHECK_NOTNULL(fsa_out);
  kk::Connection connection(fsa_in);
  kk::Array2Size<int32_t> fsa_size;
...
/*
  ...
  will be released automatically if `fsa_out`is out of scope.
 */
static void ArcSort(const kk::Fsa &fsa_in, kk::FsaCreator *fsa_out,
                    std::vector<int32_t> *arc_map = nullptr) {
  CHECK_NOTNULL(fsa_out);
  kk::ArcSorter sorter(fsa_in);
  kk::Array2Size<int32_t> fsa_size;
 ...
 */
static bool Intersect(const kk::Fsa &a, const kk::Fsa &b, kk::FsaCreator *c,
                      std::vector<int32_t> *arc_map_a = nullptr,
                      std::vector<int32_t> *arc_map_b = nullptr) {
  CHECK_NOTNULL(c);
  kk::Intersection intersection(a, b);
  kk::Array2Size<int32_t> fsa_size;
 ...
 */
static bool RandomPath(const kk::Fsa &fsa_in, bool no_eps_arc,
                       kk::FsaCreator *path,
                       std::vector<int32_t> *arc_map = nullptr) {
  CHECK_NOTNULL(path);
  kk::RandPath rand_path(fsa_in, no_eps_arc);
  kk::Array2Size<int32_t> fsa_size;
