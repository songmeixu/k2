...
void Determinize(const Ragged3<Arc> &input, const Array1<float> &input_scores,
  ...
  // 'state_subset_arcs' contains the index of the incoming arc that created it,
  // or -1 if this is the start state.
  ...
  // representation of that input-state. See also `output_fsa_idxs` and
  // `symbol_seq`.
  ...
  while (1) {
    ...
    struct Filter1 {
      __device__ pair<int, int> operator()(
          typename Array3<int>::iterator iter) const {
    ...
    // (i.e. the indexes into input.elems) but it is not itself flat; it has the
    // same 3d structure as `input`, so when indexed with a pair<int,int> it
    // gives us a 1-d array of int, hence the extra level of array here.
    //
    ...
    Array3<int> arc_idxs = input.flat_indexes()[FilteredArray3<pair<int, int>>(
    ...
    Array1<int> state_subsets_flat_indexes =
        FilteredArray2<int>(arc_idxs, ToStateSubsetsFilter()).elems() + ...;
    ...
    struct ArcToDestStateFilter {
      ...
      int operator()(typename Array3<int>::accessor acc) const {
        ...
        // like this, would be better to store src_state, dest_state and so on
        // in separate arrays.
    ...
    Array3<int> next_state =
        FilteredArray3<int>(input.elems[arc_idxs], arc_to_dest_state_filter);
    ...
    // but the mapping of indexes.  (These are the flat indexes, i.e. into the
    // .elems.)
    ...
    // arc_idxs2.elems.
    Array1<int> state_subsets_flat_indexes2 =
        state_subset_flat_indexes[sort_idxs];

    // Partition sub-lists of `arc_idxs2`, so that those with the same
    // next-state form individual sub-lists. Same underlying data.
    Array3<int> arc_idxs3 = arc_idxs2.partition(next_state2.elems[sort_idxs]);

    // Get the scores with the same structures as arc_idxs3...
    Array3<int> this_scores(arc_idxs3.shape(),
                            input_scores.elems[arc_idxs3.elems]);

    // Get sort_idxs, for reordering `this_scores` within each sub-sub-list so
    // that the one with the best score is first, and if there is a tie on
    // scores, disambiguate with arc-index.  (Would give the sort routine a
    // comparator object).
    ...
    Array1<int> state_subsets_flat_indexes3 =
    ...
    */
    ...
    Array1<int> repr_idx(repr_hash.size());
    {
      ...
      /*
        ...
        of pair<int64_t, int> (key-value pairs); the value is the index into
        state_repr.
      ...
      */
      ...
      Array1<int> new_state_nums = range(next_ostate, next_ostate + num_new);
