...
class TestWfsa(unittest.TestCase):
    ...
    def test_max_weight(self):
        ...
        self.assertTrue(
            torch.equal(forward_max_weights.data, expected_forward_max_weights)
        )
        self.assertTrue(
            torch.allclose(
                forward_max_weights.data, expected_forward_max_weights
            )
        )
        self.assertTrue(
            torch.allclose(
                backward_max_weights.data, expected_backward_max_weights
            )
        )
    ...
    def test_logsum_weight(self):
        ...
        expected_forward_logsum_weights = torch.DoubleTensor(
            [0, 1, 3, 4, 1, float("-inf"), 3, 9.126928, 4, 14.143222]
        )
        ...
        self.assertTrue(
            torch.allclose(
                forward_logsum_weights.data, expected_forward_logsum_weights
            )
        )
        self.assertTrue(
            torch.allclose(
                backward_logsum_weights.data, expected_backward_logsum_weights
            )
        )
