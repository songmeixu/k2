...
void PybindArray2Tpl(py::module &m, const char *name) {
  using PyClass = kk::DLPackArray2<Ptr, IsPrimitive, I>;
  using Parent = kk::Array2<Ptr, I>;
...
void PybindArray(py::module &m) {
  ...
  // internally so that it knows `kk::DLPackArray2`
  // is a subclass of `kk::Array2`.
  py::class_<kk::Array2<int32_t *>>(m, "_IntArray2");
  PybindArray2Tpl<int32_t *, true>(m, "DLPackIntArray2");

  // note there is a type cast as the underlying Tensor is with type `float`
  py::class_<kk::Array2<std::pair<int32_t, float> *>>(m, "_LogSumArcDerivs");
