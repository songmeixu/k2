...
void PybindFsa(py::module &m) {
  ...
  // so that it knows `kk::DLPackFsa` is a subclass of `kk::Fsa`.
  py::class_<kk::Fsa>(m, "_Fsa");

  using PyClass = kk::DLPackFsa;
  py::class_<PyClass, kk::Fsa>(m, "DLPackFsa")
